...
class PlayerScoredEventArgs {
  ...
  print() {
    console.log(
      `${this.playerName} has scored their` +
      `${this.goalsScoredSoFar} goals`
    );
  }
}
  ...
  constructor(game) {
    game.events.subscribe(function (sender, args) {
      if (args instanceof PlayerScoredEventArgs) {
        if (args.goalsScoredSoFar < 3) {
          console.log(`coach says: well done, ${args.playerName}`);
        } else {
          console.log(`coach says: congratulations, ${args.playerName}`);
        }
      }
    })
  }
